    ...
    <div className={s.content}>
      <div className={s.inner}>
        <div className={s.header}>
          ...
          {isAccount && (
            <div
              ...
              onClick={toggleIsRepliesShowing}
            >
              {APP_TEXT.myRepliesList.title}
            </div>
          )}
        </div>
        ...
        {listItems.length !== 0 ? (
                  ...
                  <div className={s.left}>
                    <div className={s.list__title}>
                      {tender.contractorId === userId ? (
                        <NavLink to={`/account/tenders/${tender.id}`}>
                          {tender.name}
                        </NavLink>
                      ) : (
                        <NavLink to={`/home/tenders/${tender.id}`}>
                          {tender.name}
                        </NavLink>
                      )}
                    </div>
                    {isAccount && (
                      <div className={s.subtitle}>{tender.status}</div>
                    )}
                    {!isAccount && (
                      ...
                      <div className={s.startBtn}>
                        {tender.status === FEATURED && (
                          <Button
                            btnColor={"#F4F4F4"}
                            textColor={"#000"}
                            onClick={() => startTender(tender.id, userId)}
                          >
                            {start}
                          </Button>
                        )}
                        {tender.status === ONGOING && (
                          <Button
                            btnColor={"#F4F4F4"}
                            textColor={"#000"}
                            onClick={() => cancelTender(tender.id, userId)}
                          >
                            {cancel}
                          </Button>
                        )}
                        {statuses.includes(tender.status) && (
                          <Button
                            btnColor={"#F4F4F4"}
                            textColor={"#000"}
                            onClick={() => retender(tender.id, userId)}
                          >
                            {retenderText}
                          </Button>
                        )}
          ...
          <div className={s.noTenders}>{APP_TEXT.tenderList.noTenders}</div>
        )}
      </div>
    </div>
  );
